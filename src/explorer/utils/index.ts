...
export const getComponentDoc = (componentName: string): ComponentDoc | undefined => {
  ...
  for (let i = 0; i < docComponents.length; i++) {
    component = docComponents[i].children.find(
      ({ name }: { name: string }) => name === componentName,
    );
    if (component) {
      break;
    }
  ...
  component.signatures?.[0].parameters[0].type.declaration.children.forEach(
    ({ name, type, defaultValue }: { name: string; type: any; defaultValue: any }) => {
      if (name === 'children') {
